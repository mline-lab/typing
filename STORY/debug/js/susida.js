...
var wordList_jp = [
	"ハジライ","堀田","MRP","帰りたい帰りたい帰りたい","スクリプト","スライムタワー","就職情報センター",
	"ご飯前には手洗いを","ハジライも手洗いを","布団でパン","俺の絵が一番かわいい","マナーモードハジライ","!?","、。",
    "細菌","コンビニでブラックサンダー","食中毒","抗生物質","免疫","集団食中毒事件","国民生活センター",
    "感染症","睡眠","感染予防のアロマセラピー","ビタミンサラダを摂取","こまめに緑茶を飲む","寒冷刺激を避ける","禁煙",
    "栄養と休養","過度な緊張感","ノロウイルス","ロタウイルス","病原体","飛沫感染","経口感染",
    "接触感染","微生物","空気感染","傷口感染","二次感染","家庭内感染","手洗いでは菌はゼロにならない",
    "破傷風菌","サルモネラ菌","カンピロバクター","マイコプラズマ","潜伏期間","微生物","酵母",
    "アオカビ","コウジカビ","クラドスポリウム","感染の予防が大切","食品はよく洗い十分加熱","肉にはよく火を通す","生食のものはよく洗う"
];

var wordList_hiragana = [
	"はじらい","ほった","mrp","かえりたいかえりたいかえりたい","すくりぷと","すらいむたわー","しゅうしょくじょうほうせんたー",
	"ごはんまえにはてあらいを","はじらいもてあらいを","ふとんでぱん","おれのえがいちばんかわいい","まなーもーどはじらい","!?",",.",
    "さいきん","こんびにでぶらっくさんだー","しょくちゅうどく","こうせいぶっしつ","めんえき","しゅうだんしょくちゅうどくじけん","こくみんせいかつせんたー",
    "かんせんしょう","すいみん","かんせんよぼうのあろませらぴー","びたみんさらだをせっしゅ","こまめにりょくちゃをのむ","かんれいしげきをさける","きんえん",
    "えいようときゅうよう","かどなきんちょうかん","のろういるす","ろたういるす","びょうげんたい","ひまつかんせん","けいこうかんせん",
    "せっしょくかんせん","びせいぶつ","くうきかんせん","きずぐちかんせん","にじかんせん","かていないかんせん","てあらいではきんはぜろにならない",
    "はしょうふうきん","さるもねらきん","かんぴろばくたー","まいこぷらずま","せんぷくきかん","びせいぶつ","こうぼ",
    "あおかび","こうじかび","くらどすぽりうむ","かんせんのよぼうがたいせつ","しょくひんはよくあらいじゅうぶんかねつ","にくにはよくひをとおす","なましょくのものはよくあらう"
];
*/
...
window.onload = function (){
	...
	select 						 = document.getElementById("word_select");
...
function csv_load(){
	$("#word_select").empty();
	$("#word_select").append('<option value="0"> 選択してください </option>');
	for(var i=0;i<wordList_jp.length;i++){
			$("#word_select").append('<option value="' +i+ '">' +wordList_jp[i]+ '</option>');
	}
}

$("#word_select").change(function(){
	choise = $("#word_select option:selected").val();
});
...
function set_audio(){
    ...
    audioElem.src		 		= "audio/syan.ogg";
		...
		document.getElementById("debug").innerHTML= "♪~純愛レンズ/東條 希(CV.楠田 亜衣奈)~";
    ...
    audioElem.src 			= "audio/se_ok.ogg";
		...
		document.getElementById("debug").innerHTML= "♪~M♭/加藤 恵(安野 希世乃)~";
...
function reward(){
 ...
 //別窓の左と上の余白を求める
  ...
  var h = ( screen.height-360 ) / 2;


  //オプションパラメーターleftとtopに余白の値を指定
  window.open("reward/nozomi.html","","width=640,height=360"+",left="+w+",top="+h);
}
...
function nextWord(){
	...
	charIndex 	= 0;
	var random 	= Math.floor( Math.random() * wordList_hiragana.length );

	wordArea_hiragana.textContent = wordList_hiragana[choise];
	wordArea_jp.textContent   		= wordList_jp[choise];

	//ひらがなを変換
	worddata	=	townro_machange(wordList_hiragana[choise],henkan);
	ro_ma			=	worddata[0];
	jword			=	worddata[1];
	word			=	worddata[2];
...
//配列データを個々のデータに入れ直す
	...
	henkan				=	data[10];townro_machange
	ro_ma					=	data[11];
	shiftdown			=	data[13];
if (game_flag == 1) {
...
if (game_flag == 1) {
	...
	//正解文字を入力した時
		...
		if( gauge <= 10){
			gauge = gauge + 1;
			switch(gauge){
				case 1:charge.textContent = "●○○○○○○○○○";
					break;
				case 2:charge.textContent = "●●○○○○○○○○";
					break;
				case 3:charge.textContent = "●●●○○○○○○○";
					break;
				case 4:charge.textContent = "●●●●○○○○○○";
					break;
				case 5:charge.textContent = "●●●●●○○○○○";
					break;
				case 6:charge.textContent = "●●●●●●○○○○";
					break;
				case 7:charge.textContent = "●●●●●●●○○○";
					break;
				case 8:charge.textContent = "●●●●●●●●○○";
					break;
				case 9:charge.textContent = "●●●●●●●●●○";
					break;
				case 10:charge.textContent = "●●●●●●●●●●";
					break;
			}
